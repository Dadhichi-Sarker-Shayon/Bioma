...
const SpeciesImage = ({ src, alt, style }) => {
  ...
  if (error || !imgSrc) {
    return (
      <div style={{
        ...
        color: 'white', fontSize: '3rem', fontWeight: 700, ...style,
      }}>
  ...
  return (
    <img src={imgSrc} alt={alt} style={{ width: '100%', height: '100%', objectFit: 'cover', ...style }}
      onError={() => setError(true)} crossOrigin="anonymous" referrerPolicy="no-referrer" />
  );
};
...
const SpeciesDetail = () => {
  ...
  useEffect(() => { fetchSpeciesDetail(); }, [id]);
  ...
  const fetchSpeciesDetail = async () => {
    ...
    try {
      ...
      setError(err.response?.status === 404 ? 'Species not found.' : 'Failed to load species details.');
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '2rem', background: '#f0f2f5', minHeight: '100vh' }}>
      ...
      {/* Hero Image */}
      {species.imageUrl && (
        <div style={{ width: '100%', height: '350px', borderRadius: '8px', overflow: 'hidden', marginBottom: '1.5rem', border: '1px solid #dde1e6' }}>
          <SpeciesImage src={species.imageUrl} alt={species.commonName || species.scientificName} />
        </div>
      )}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
            ...
            {species.statusCode && (
              <span style={{
                ...
                background: `${getStatusColor(species.statusCode)}15`, color: getStatusColor(species.statusCode),
                border: `1px solid ${getStatusColor(species.statusCode)}30`,
              }}>{species.statusCode}</span>
            )}
          </div>
          ...
          {species.commonName && (
            <h2 style={{ fontSize: '1.1rem', color: '#888', fontStyle: 'italic', fontWeight: 400 }}>
              {species.scientificName}
            </h2>
          )}
        </div>
        {species.discoveryYear && (
          <div style={{ background: 'white', border: '1px solid #dde1e6', borderRadius: '6px', padding: '0.75rem 1.25rem', textAlign: 'center' }}>
            <div style={{ fontSize: '0.65rem', color: '#888', textTransform: 'uppercase' }}>Discovered</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.5rem' }}>
          ...
          {(species.physicalDescription || species.avgHeightCm || species.avgLengthCm) && (
            <section style={{ background: 'white', border: '1px solid #dde1e6', borderRadius: '6px', padding: '1.5rem' }}>
              ...
              <div style={{ display: 'flex', gap: '2rem', marginBottom: '1rem' }}>
                {species.avgHeightCm && <div><div style={{ fontSize: '0.7rem', color: '#888', textTransform: 'uppercase' }}>Height</div><div style={{ fontWeight: 600 }}>{species.avgHeightCm} cm</div></div>}
                {species.avgLengthCm && <div><div style={{ fontSize: '0.7rem', color: '#888', textTransform: 'uppercase' }}>Length</div><div style={{ fontWeight: 600 }}>{species.avgLengthCm} cm</div></div>}
              </div>
            ...
            <section style={{ background: 'white', border: '1px solid #dde1e6', borderRadius: '6px', padding: '1.5rem' }}>
              ...
              {species.dietType && (
                <div style={{ marginBottom: '1rem' }}>
                  <span className="badge badge-success" style={{ fontSize: '0.75rem' }}>{species.dietType}</span>
            ...
            <section style={{ background: 'white', border: '1px solid #dde1e6', borderRadius: '6px', padding: '1.5rem' }}>
              ...
              <div style={{ display: 'grid', gap: '0.75rem' }}>
                {species.distributions.map((dist, i) => (
                  <div key={i} style={{ padding: '0.75rem', borderRadius: '6px', border: '1px solid #e8eaed', background: '#f8f9fa' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          {species.funFact && (
            <div style={{ background: 'white', border: '1px solid #dde1e6', borderLeft: '3px solid #f39c12', borderRadius: '6px', padding: '1.25rem' }}>
